...
async def edit_event_confirm(call: CallbackQuery, state: FSMContext, session: AsyncSession,
    ...
    await bot_messages.add_event(call=call, state=state, session=session, scheduler=scheduler)
...
async def search_events(call: CallbackQuery, session: AsyncSession) -> None:
    data = await requests.select_event(session=session, event_id=call.data[1:])
