...
class LogAllEventsMiddleware(BaseMiddleware):
    ...
    def _create_log_record(
        self, handler: HandlerObject, event: Union[CallbackQuery, Message], data: Dict[str, Any], *, duration: Optional[float] = None
    ) -> logging.LogRecord:
        callback = handler.callback
        func_name = callback.__name__
        pathname = inspect.getsourcefile(callback) or ""
        ...
        if isinstance(event, Message):
            if event.text:
                message_text = (f"{event.text[:50]}..." if len(event.text) > 50 else event.text) or ""
        ...
        if duration is not None:
            msg = f"Handler `{func_name}` Duration {int(duration * 1000)} ms: {msg}"

        record = logging.LogRecord(
            name="c.bot.middlewares.LogAllEventsMiddleware",
            level=logging.INFO,
            pathname=pathname,
            lineno=lineno,
            msg=msg,
